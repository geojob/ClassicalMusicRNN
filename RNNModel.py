...
def Input_Kernel(input_data, Midi_low, Midi_high, time_init):
    ...
    # if input_data.shape[0] is None:
    #     return tf.keras.layers.Input(shape=(None,90,128,80))
    
    # Capture input_data dimensions (batch_size and num_timesteps are variable length)
    batch_size = input_data.shape[0]
    num_notes = input_data.shape[1]
    num_timesteps = input_data.shape[2]
...
def LSTM_TimeWise_Training_Layer(input_data, state_init, output_keep_prob=1.0):
  ...
  """  
    ...
    # if input_data.shape[1] is None:
    #     return tf.keras.layers.Input(shape=(None,90,128,200))
    ...
    for h in range(num_layers):
        num_states.append(state_init[h][0].shape[1])
        lstm_cell = tf.keras.layers.LSTMCell(num_states[h],dropout=output_keep_prob)
        cell_list.append(lstm_cell)


    #Run through LSTM time steps and generate time-wise sequence of outputs
    multi = tf.keras.layers.StackedRNNCells(cell_list)
    #RNN = tf.keras.layers.RNN(multi)
    #print(input_flatten.shape)
    #print(state_init)
    output_flat, state_out,*_ = tf.keras.layers.RNN(cell=multi,return_sequences=True, return_state=True,input_shape=(1350,128,80))(inputs=input_flatten, initial_state=state_init)
...
def LSTM_NoteWise_Layer(input_data, state_init, output_keep_prob=1.0):
    ...
    for n in range(num_notes):    
        ...
        #print('Cell inputs shape = ', cell_inputs)
        #print(h_state)
    
        # output shape = [batch_size*num_timesteps, Nfinal] 
        h_final_out, h_state = multi(inputs=cell_inputs, states=h_state)       
        ...
        y_n = tf.keras.layers.Dense(units=2, activation=None)(h_final_out)
        #print('y_n shape = ', y_n.get_shape())
        #print(y_n.numpy)

        # sample note play/articulation using Probability of event = sigmoid(y_n)
        probs = 1/1+np.exp(-y_n)
        #print(len(probs))
        number_of_rows = probs.shape[0]
        random_indices = np.random.choice(number_of_rows, size=len(probs), replace=False)
        random_rows = probs[random_indices, :]
        note_gen_n = random_rows
        #print("This is note",note_gen_n)
        #note_gen_n = tfp.distributions.Bernoulli(logits=y_n).sample()      
...
def Loss_Function(Note_State_Batch, y_out):
    ...
    entries in y_out at time step $t$ to predict Note_State_Batch at time step $t+1$.  
    """   

    
    # batch_size and num_timesteps are variable length
    batch_size = y_out.shape[0]
    num_notes = y_out.shape[1]
    num_timesteps = y_out.shape[2]
    ...
    #print(y_align.dtype)
    #print(Note_State_Batch_align.dtype)
